...
namespace chameleon {
    ...
    /// count_display_renderer handles openGL calls for a display.
    class count_display_renderer : public QObject, public QOpenGLFunctions_3_3_Core {
        Q_OBJECT
        public:
        count_display_renderer(QSize canvas_size, float discard_ratio, std::size_t colormap) :
            _canvas_size(std::move(canvas_size)),
            _discard_ratio(discard_ratio),
            _colormap(colormap),
            _counts(_canvas_size.width() * _canvas_size.height(), std::numeric_limits<uint32_t>::max()),
            _discards_changed(false),
            _automatic_calibration(true),
            _program_setup(false) {
            _accessing_counts.clear(std::memory_order_release);
            _accessing_discards.clear(std::memory_order_release);
        }
        count_display_renderer(const count_display_renderer&) = delete;
        count_display_renderer(count_display_renderer&&) = delete;
        count_display_renderer& operator=(const count_display_renderer&) = delete;
        count_display_renderer& operator=(count_display_renderer&&) = delete;
        virtual ~count_display_renderer() {
        ...
        void push(Event event) {
            ...
            while (_accessing_counts.test_and_set(std::memory_order_acquire)) {
            }
            _counts[index] = static_cast<uint32_t>(event.count);
            _accessing_counts.clear(std::memory_order_release);
        }
        ...
        void paint() {
            ...
            if (!_program_setup) {
                ...
                glBufferData(
                    ...
                    _counts.size() * sizeof(decltype(_counts)::value_type),
                    nullptr,
                    GL_DYNAMIC_DRAW);
                ...
                0);
            {
                ...
                std::copy(_counts.begin(), _counts.end(), buffer);
                _accessing_counts.clear(std::memory_order_release);
                while (_accessing_discards.test_and_set(std::memory_order_acquire)) {
                }
                if (_automatic_calibration) {
                    auto previous_discards = _discards;
                    std::vector<uint32_t> sorted_counts(_counts.size());
                    const auto end =
                        std::copy_if(buffer, buffer + _counts.size(), sorted_counts.begin(), [](uint32_t count) {
                            return count > 1;
                        });
                    const auto size = static_cast<std::size_t>(std::distance(sorted_counts.begin(), end));
                    if (size > 0) {
                        std::sort(sorted_counts.begin(), end);
                        auto white_discard_candidate =
                            sorted_counts[static_cast<std::size_t>(size * (1.0f - _discard_ratio))];
                        auto black_discard_candidate =
                            sorted_counts[static_cast<std::size_t>(size * _discard_ratio + 0.5f)];
                        if (white_discard_candidate > black_discard_candidate) {
                            _discards.setX(black_discard_candidate);
                            _discards.setY(white_discard_candidate);
                        } else {
                            black_discard_candidate = sorted_counts.front();
                            white_discard_candidate = *std::prev(end);
                            if (white_discard_candidate > black_discard_candidate) {
                                _discards.setX(black_discard_candidate);
                                _discards.setY(white_discard_candidate);
                            }
                ...
                {
                    const auto delta = std::log(_discards.y() / _discards.x());
        ...
        std::atomic_flag _accessing_counts;
        QRectF _clear_area;
        QRectF _paint_area;
        QVector2D _discards;
    ...
    /// count_display displays a stream of exposure measurements encoded as time differences.
    class count_display : public QQuickItem {
        Q_OBJECT
        ...
        count_display() :
            ...
            connect(this, &QQuickItem::windowChanged, this, &count_display::handle_window_changed);
            _accessing_renderer.clear(std::memory_order_release);
        }
        count_display(const count_display&) = delete;
        count_display(count_display&&) = delete;
        count_display& operator=(const count_display&) = delete;
        count_display& operator=(count_display&&) = delete;
        virtual ~count_display() {}
        ...
        virtual void set_discards(QVector2D discards) {
            ...
            if (_renderer_ready.load(std::memory_order_relaxed)) {
                _count_display_renderer->set_discards(discards);
            } else {
        ...
        void push(Event event) {
            ...
            _count_display_renderer->push<Event>(event);
        }
        ...
        void assign(Iterator begin, Iterator end) {
            ...
            _count_display_renderer->assign<Iterator>(begin, end);
        }
        ...
        void sync() {
            if (_ready.load(std::memory_order_acquire)) {
                if (!_count_display_renderer) {
                    _count_display_renderer = std::unique_ptr<count_display_renderer>(new count_display_renderer(
                        _canvas_size, _discard_ratio, static_cast<std::size_t>(_colormap)));
                    connect(
                        window(),
                        &QQuickWindow::beforeRendering,
                        _count_display_renderer.get(),
                        &count_display_renderer::paint,
                        Qt::DirectConnection);
                    connect(
                        _count_display_renderer.get(),
                        &count_display_renderer::discards_changed,
                        this,
                        &count_display::update_discards);
                    while (_accessing_renderer.test_and_set(std::memory_order_acquire)) {
                    }
                    _count_display_renderer->set_discards(_discards_to_load);
                ...
                if (clear_area != _clear_area) {
                    ...
                    _count_display_renderer->set_rendering_area(
        ...
        void handle_window_changed(QQuickWindow* window) {
            if (window) {
                connect(window, &QQuickWindow::beforeSynchronizing, this, &count_display::sync, Qt::DirectConnection);
                connect(
                    window,
                    &QQuickWindow::sceneGraphInvalidated,
                    this,
                    &count_display::cleanup,
                    Qt::DirectConnection);
        ...
        float _discard_ratio;
        Colormap _colormap;
        std::unique_ptr<count_display_renderer> _count_display_renderer;
        QRectF _clear_area;
        QRectF _paint_area;
    };
}
