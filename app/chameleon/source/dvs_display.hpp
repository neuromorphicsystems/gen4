...
namespace chameleon {
    ...
    class dvs_display_renderer : public QObject, public QOpenGLFunctions_3_3_Core {
        ...
        dvs_display_renderer(
            ...
            for (std::size_t style = 0; style < _style_to_fragment_shader.size(); ++style) {
                ...
                auto add_color_to_stream = [&](QColor color) {
                    fragment_shader_stream << "    vec4(" << color.redF() << ", " << color.greenF() << ", "
                ...
                if (on_colormap.size() < off_colormap.size()) {
                    for (std::size_t index = 0; index < static_cast<std::size_t>(off_colormap.size() - on_colormap.size()); ++index) {
                        fragment_shader_stream << ",\n";
                        add_color_to_stream(on_colormap.back());
                    }
                }
                ...
                if (off_colormap.size() < on_colormap.size()) {
                    for (std::size_t index = 0; index < static_cast<std::size_t>(on_colormap.size() - off_colormap.size()); ++index) {
                        fragment_shader_stream << ",\n";
                ...
                switch (style) {
                    case 0:
                        ...
                        break;
                    case 1:
                        fragment_shader_stream << "    float lambda = (current_t - t_and_on.x) < parameter ? "
                                                  "(current_t - t_and_on.x) / parameter : 1.0f;\n";
                        break;
                    case 2:
                        fragment_shader_stream
                            << "    float lambda = (current_t - t_and_on.x) < parameter ? 0.0f : 1.0f;\n";
                        break;
                    default:
            ...
            _accessing_style.clear(std::memory_order_release);
        ...
        void push_unsafe(Event event) {
            ...
            while (event.t - offset_t > static_cast<uint64_t>(std::numeric_limits<uint32_t>::max())) {
                offset_t += std::numeric_limits<uint32_t>::max() / 2;
                for (std::size_t index = 0; index < _ts_and_ons.size(); index += 2) {
                    if (_ts_and_ons[index] > std::numeric_limits<uint32_t>::max() / 2) {
                        _ts_and_ons[index] -= std::numeric_limits<uint32_t>::max() / 2;
                    } else {
                        _ts_and_ons[index] = 0;
                    }
                }
            }
            _ts_and_ons[index] = static_cast<uint32_t>(event.t - offset_t);
            _ts_and_ons[index + 1] = event.on ? 1 : 0;
            _current_t = static_cast<uint32_t>(event.t - offset_t);
        }
        ...
        void set_style(std::size_t style) {
            ...
            while (_accessing_style.test_and_set(std::memory_order_acquire)) {
            }
            _style = style;
            _accessing_style.clear(std::memory_order_release);
        }
        ...
        void paint() {
            ...
            if (!_program_setup) {
                ...
                for (std::size_t style = 0; style < _style_to_program_id.size(); ++style) {
                    ...
                    glUniform1f(glGetUniformLocation(_style_to_program_id[style], "width"), static_cast<GLfloat>(_canvas_size.width()));
                    glUniform1f(
                        glGetUniformLocation(_style_to_program_id[style], "height"), static_cast<GLfloat>(_canvas_size.height()));
                    _current_t_location = glGetUniformLocation(_style_to_program_id[style], "current_t");
                    _parameter_location = glGetUniformLocation(_style_to_program_id[style], "parameter");
                ...
                0);
            {
                ...
                glUniform1f(
                    _parameter_location,
                    static_cast<GLfloat>(_parameter.load(std::memory_order_relaxed) * (style == 1 ? 2.0f : 1.0f)));
                while (_accessing_ts_and_ons.test_and_set(std::memory_order_acquire)) {
                }
